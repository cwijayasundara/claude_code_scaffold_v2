...
2. **Read the execution plan** — check for an approved plan if one exists
3. **Read the architecture** — check `.claude/docs/architecture.md` and `CLAUDE.md` for layer rules
...
8. **Run linters** — execute `bash .claude/lint_all.sh` and fix any violations
...
- Update `.claude/docs/` if you change public interfaces
